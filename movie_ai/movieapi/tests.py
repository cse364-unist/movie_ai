...
class MovieAPITestCase(APITestCase):
    ...
    def test_short_form_3(self):
        response = self.client.post(reverse('short_form'), {}, format='json')
    ...
    def test_video_qa_3(self):
        response = self.client.post(reverse('video_qa'), {}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_avatar_chat_3(self):
        response = self.client.post(reverse('avatar_chat'), {"character": "Witch"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
